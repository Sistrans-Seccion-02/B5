...
public class BaresController {
    ...
    public String barGuardar(@ModelAttribute Bar bar){
        barRepository.insertarBar(bar.getEstilo(), bar.getAforo(), bar.getAplicaCompartido());
    ...
    public String barEditarGuardar(@PathVariable("id") int id, @ModelAttribute Bar bar){
        barRepository.actualizarBar(id, bar.getEstilo(), bar.getAforo(), bar.getAplicaCompartido());
        return "redirect:/bares";
    }
